...
struct admissible_graph{
	...
	// tr must be a tree
	// tr_size[u]: size of the subtree of u
...
int main(){
	...
	int n, m;
	cin >> n >> m;
	vector<vector<vector<int>>> trees(m);
	for(auto i = 0; i < m; ++ i){
		int k = n;
		trees[i].resize(k);
		for(auto j = 0; j < k - 1; ++ j){
			...
			trees[i][v].push_back(u);
		}
	}
	admissible_graph AG(n);
	auto edge = AG.get_edge();
	assert((int)edge.size() <= 6901);
	cout << (int)edge.size() << "\n";
	for(auto [u, v]: edge){
		cout << u + 1 << " " << v + 1 << "\n";
	}
	for(auto tree: trees){
		auto res = AG.embed(tree);
		for(auto x: res){
			cout << x + 1 << " ";
		}
		cout << "\n";
